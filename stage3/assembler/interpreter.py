...
class UVMInterpreter:
    ...
    def execute_instruction(self, opcode: int, field_b: int, field_c: int, field_d: Optional[int]) -> None:
        ...
        try:
            if opcode == 72:  # Загрузка константы
                ...
                # Пока не реализован на этом этапе
                print(f"Предупреждение: команда унарного минуса (код 91) еще не реализована")
                self.pc += 4
    ...
    def run(self) -> None:
        ...
        while not self.halted:
            ...
            if self.instructions_executed > 1000000:
                print("Превышено максимальное количество команд (1,000,000)")
                self.halted = True
